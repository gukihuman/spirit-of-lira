...
export default defineNuxtPlugin(async () => {
    ...
    await load(import.meta.glob("@/assets/**/*"), ASSETS.md_paths, "md")
    await load(import.meta.glob("@/assets/**/*"), ASSETS.mp3_paths, "mp3")
    await load(import.meta.glob("@/assets/**/*"), ASSETS.webp_paths, "webp")
    await load(import.meta.glob("@/assets/**/*"), ASSETS.jsons, "json")
    // update image path for sprite jsons, super mega essentially important :)
    _.forEach(ASSETS.jsons, (value, name) => {
        if (value.meta && ASSETS.webp_paths[name]) {
            value.meta.image = ASSETS.webp_paths[name]
        }
    })
})
